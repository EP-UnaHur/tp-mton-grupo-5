...
exports.getAllCarreras = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(carreras.length > 0 ? carreras : "No hay carreras agregados");
  } catch (error) {
...
exports.crearMateria = async (req, res) => {
  try {
    ...
    if (carrera) {
      ...
      const newRecord = await Materia.create({
...
exports.getMaterias = async (req, res) => {
  try {
    const carrera = await Carreras.findByPk(req.params.id);
    if (carrera) {
      const carreraConMaterias = await Carreras.findByPk(req.params.id, {
